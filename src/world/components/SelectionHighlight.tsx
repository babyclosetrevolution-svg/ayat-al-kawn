...
export function SelectionHighlight() {
  ...
  useFrame((state, delta) => {
    ...
    const targetScale = rec ? rec.distance / 4 : 0;
    ...
    // Always face the camera, with a gentle pulse.
    ring.lookAt(state.camera.position);
    const pulse = 0.55 + Math.sin(state.clock.elapsedTime * 1.6) * 0.08;
    mat.opacity = rec ? pulse * 0.35 : 0;
  });

  return (
    <mesh ref={ringRef} renderOrder={2}>
      <ringGeometry args={[1.4, 1.55, 96]} />
      <meshBasicMaterial
        ref={matRef}
        color="#9fc4ff"
        transparent
